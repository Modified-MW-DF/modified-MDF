...
local addMinionToCiv = dfhack.script_environment('AddPetToCiv')
...
local function addfbtopets(civ)
	local caste, id, raw

	for id, raw in pairs(df.global.world.raws.creatures.all) do
		if starts(raw.creature_id, 'FORGOTTEN_BEAST_') then
...
if not args.civ then
	qerror('fbspets: No civ designated')
end

if not moduleMode then
	addfbtopets(args.civ)
end
